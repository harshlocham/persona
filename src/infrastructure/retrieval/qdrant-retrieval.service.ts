...
export class QdrantRetrievalService implements RetrievalPort {
  ...
  async retrieve(request: RetrievalRequest): Promise<RetrievalResult> {
    ...
    try {
      ...
      const resourceFilter = request.resourceType
        ? {
            must: [
              { key: "persona", match: { value: request.personaId } },
              { key: "type", match: { value: request.resourceType } },
            ],
          }
        : personaFilter;
      ...
      const [knowledgeHits, resourceHits] = await Promise.all([
        ...
        request.wantsResources
          ? client
              .search(env.RESOURCES_COLLECTION, {
                vector,
                limit: RESOURCE_TOP_K,
                filter: resourceFilter,
                with_payload: true,
              })
              .catch(() => [] as QdrantSearchHit[])
          : Promise.resolve([] as QdrantSearchHit[]),
      ]);
